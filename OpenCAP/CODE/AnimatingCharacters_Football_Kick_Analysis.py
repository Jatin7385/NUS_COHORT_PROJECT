...
markerData1 = readMarkerData("C:/Users/Jatin/Desktop/Engineering/ROBOTICS PROGRAM NUS/PROJECT/OpenCAP/DATA/NUS_PROJECT_DATA/MarkerData/Football_Full_Motion.trc")
n1 = markerData['NumFrames']

def getCoordinates(i, markerData):
    X = []
    Y = []
    Z = []
    Labels = []
...
for i in range(int(n)):
    ...
    X,Y,Z,Labels = getCoordinates(i, markerData)
    X1,Y1,Z1,Labels1 = getCoordinates(i, markerData1)

    # Trim the longer arrays to match the length of the shorter one
    min_length = min(len(X), len(X1))
    X = X[:min_length]
    Y = Y[:min_length]
    Z = Z[:min_length]
    ...
    for segment in line_segments:
        ...
        ax.plot(x_conn, y_conn, z_conn, c='b', linestyle='-', linewidth=2, label = "Not Optimal Elbow rotation")


    # Scatter3d trace for lines
    for segment in line_segments:
        x_conn_1 = [x_points1[j] for j in segment]
        y_conn_1 = [y_points1[j] for j in segment]
        z_conn_1 = [z_points1[j] for j in segment]
        ax.plot(x_conn_1, y_conn_1, z_conn_1, c='r', linestyle='-', linewidth=2, label = "Optimal Elbow rotation")


    # Create custom legend patches
    patch1 = mpatches.Patch(color='blue', label='Not Optimal Running')
    patch2 = mpatches.Patch(color='red', label='Optimal Running')
